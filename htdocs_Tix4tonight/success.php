...
$id_ingresso = $checkout_session->metadata->id_ingresso;
$id_cliente = $checkout_session->metadata->id_cliente;
$id_quantidade = $checkout_session->metadata->id_quant;
$valor = $checkout_session->amount_total;

echo "<hr>";

//Sql que altera um registro da tabela usuários
$sql = "INSERT INTO pedido(Cliente_idCliente, Ingresso_idIngresso, Valor, Status, Quantidade) VALUES($id_cliente, $id_ingresso, $valor, 'realizado', $id_quantidade)"; 
            
$compra = $id_quantidade;  

$results = mysqli_query($conn,"SELECT * FROM ingresso WHERE idIngresso = $id_ingresso");
...
$sql2 = "UPDATE `ingresso` SET `quantidadeTotal` = '$z' WHERE `ingresso`.`quantidadeTotal` = '$y' AND `ingresso`.`idIngresso` = $id_ingresso;
";
