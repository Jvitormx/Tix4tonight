...
$sql = "SELECT * FROM organizador ORDER BY idOrganizador DESC";
	...
	<?php
	  while($row = $result->fetch_assoc()) {
		...
		#<td><a href='eventobloquear.php?id=" . $row["id"] . "&status=" . $row["status"] . "'><img src='./imagens/bloquear.png' alt='Bloquear Evento'></a></td><td><a href='usuarioexcluir.php?id=" . $row["Id"] . "'><img src='./imagens/excluir.png' alt='Excluir Usuário'></a></td></tr>";
		 echo "<td><a href='clientebloquear.php?id=" . $row["idCliente"] . "&status=" . $row["status"] . "'><img src='./imagens/bloquear.png' alt='Bloquear cliente'></a></td>";
	  }
	?>
...
<div class="pagination">
    <?php for($i=1; $i <= $contagem; $i++) {
            echo "<a href='usuarios.php?pag=$i&idClienter=$idAdm'>$i</a>";
    } 
	?>   
</div>  
