...
function show_comments($comments, $parent_id = -1) {
    ...
    if ($parent_id != -1) {
        ...
        array_multisort(array_column($comments, 'comentario_data'), SORT_ASC, $comments);
    }
        ...
        if ($comment['parent_id'] == $parent_id) {
            ...
            <div class="comment">
                <div>
                    ...
                    <span class="date">' . time_elapsed_string($comment['comentario_data']) . '</span>
                </div>
...
function show_write_comment_form($parent_id = -1) {
    ...
    <div class="write_comment" data-comment-id="' . $parent_id . '">
        <form>
            ...
            <input name="nome" type="text" placeholder="Your Name" required>
            <textarea name="comentario" placeholder="Write your comment here..." required></textarea>
...
if (isset($_GET['Evento_idEvento'])) {
    ...
    if (isset($_POST['nome'], $_POST['comentario'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO comentario (Evento_idEvento, parent_id, nome, comentario, comentario_data) VALUES (?,?,?,?,NOW())');
    ...
    $stmt = $pdo->prepare('SELECT COUNT(*) AS total_comentarios FROM comentario WHERE Evento_idEvento = ?');
    $stmt->execute([ $_GET['Evento_idEvento'] ]);
...
<div class="comment_header">
    <span class="total"><?=$comments_info['total_comentarios']?> comments</span>
