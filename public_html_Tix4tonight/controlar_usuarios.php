...
$sql = "SELECT * FROM cliente ORDER BY idCliente DESC LIMIT $id, $total";
...
<tr><th>ID</th><th>Nome</th><th>Email</th><th>Acesso</th><th>Status</th><th colspan="3">Ações</td></tr>
	...
	<?php
	  while($row = $result->fetch_assoc()) {
		...
		echo "<td>" . $row["idCliente"] . "</td><td>" . $row["nome"] . "</td><td>" . $row["email"] . "</td><td>" . $row["acesso"] . "</td><td>" . $row["status"] . "</td>";
		#<td><a href='eventobloquear.php?id=" . $row["id"] . "&status=" . $row["status"] . "'><img src='./imagens/bloquear.png' alt='Bloquear Evento'></a></td><td><a href='usuarioexcluir.php?id=" . $row["Id"] . "'><img src='./imagens/excluir.png' alt='Excluir Usuário'></a></td></tr>";
		 echo "<td><a href='usuariobloquear.php?idCliente=" . $row["idCliente"] . "&status=" . $row["status"] . "'><img src='./imagens/cross1.png' alt='Bloquear usuario'></a></td><td><a href='excluir_cliente.php?idCliente=" . $row["idCliente"] . "'><img src='./imagens/trash1.png' alt='Excluir Cliente'></a></td>";
	  }
	?>
...
<div class="pagination">
    <?php for($i=1; $i <= $contagem; $i++) {
            echo "<a href='usuarios.php?pag=$i'>$i</a>";
    } 
	?>   
</div>  
